...
import { ShieldCheck, Zap, TrendingUp, ArrowRight, Sparkles, Hexagon, Cpu } from "lucide-react";
...
export default function Home() {
  return (
    <div className="min-h-screen flex flex-col bg-transparent text-zinc-100 font-sans relative overflow-x-hidden">
      ...
      {/* 🧭 Navigation Header - Landing Specific */}
      <nav className="fixed top-0 w-full z-[60] border-b border-white/5 bg-black/20 backdrop-blur-xl">
        <div className="max-w-7xl mx-auto px-8 h-20 flex items-center justify-between">
          <Link href="/" className="flex items-center gap-3 group">
            <div className="relative">
              <Hexagon className="w-10 h-10 text-indigo-500 fill-indigo-500/10 group-hover:rotate-90 transition-transform duration-700" strokeWidth={1.5} />
              <Cpu className="absolute inset-0 m-auto w-5 h-5 text-indigo-400 animate-pulse" />
            </div>
            <div className="flex flex-col leading-none">
              <span className="font-black tracking-tighter text-2xl uppercase italic bg-clip-text text-transparent bg-gradient-to-r from-white via-indigo-100 to-indigo-400">
                Axiom.io
              </span>
              <span className="text-[8px] font-bold tracking-[0.4em] uppercase text-indigo-500/80 ml-0.5">
                Audit Engine
              </span>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-8">
            ...
            <Link
              href="/register"
              className="px-6 py-2.5 bg-white text-black rounded-xl text-sm font-bold hover:bg-zinc-200 transition-all active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.3)] flex items-center gap-2"
            >
              Get Started <Sparkles className="w-3.5 h-3.5 text-indigo-600" />
            </Link>
          </div>
        </div>
      </nav>
      ...
      {/* HERO CONTENT */}
      <main className="flex-1 max-w-6xl mx-auto flex flex-col items-center justify-center text-center px-6 pt-40 pb-20 relative">
